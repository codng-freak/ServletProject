...
/**
 * Defines method to display the response from server to the user. Displays the fruit items list after a particular fruit
 * item is deleted.
 * @author Harsh Sharma
 *
 */
public class DeleteFruitItemsServlet extends HttpServlet {
	...
	/**
	 * doPost method that takes request from the user regarding which fruit item has to be deleted.
	 * @param request A HttpServletRequest object
	 * @param response A HttpServletRequest object
	 * @throws ServletException Defines a general exception a servlet can throw when it encounters difficulty.
	 * @throws IOException Exceptions produced by failed or interrupted I/O operations
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
		LOGGER.info("Inside doPost method of DeleteFruitItemsServlet class");
		...
		try {
			rowsUpdateCount = fruitItemsService.deleteFruitItems(itemNum);
			if (rowsUpdateCount > 0) {
				LOGGER.info("Fruit Item is successfully deleted as per user's request.");
