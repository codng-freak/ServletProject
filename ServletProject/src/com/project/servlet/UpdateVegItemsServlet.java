...
/**
 * Defines method to request updated vegetarian items data from a user and forward that data to the service layer for further operations.
 ...
 * retrieve updated vegetarian items details from user and manipulate requests on the client side.
 * @author Harsh Sharma
 *
 */
public class UpdateVegItemsServlet extends HttpServlet {
	 ...
	 */
	static final Logger LOGGER = Logger.getLogger(UpdateVegItemsServlet.class);

	/**
	 * Method to request updated vegetarian items data from the HTML page.
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		LOGGER.info("Inside doPost method of UpdateVegItemsServlet class");
		...
		try {
			if (request.getParameter("forward") != null) {
				...
				vegItems.setItemNum(itemNum);
				int result = vegItemsService.updateVegItems(vegItems);
				if (result > 0) {
					LOGGER.info("Veg Item updated successfully for item number " + itemNum
