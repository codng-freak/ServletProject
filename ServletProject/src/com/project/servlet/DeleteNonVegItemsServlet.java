...
/**
 * Defines method to display the response from server to the user. Displays the non vegetarian items list after a particular non vegetarian
 * item is deleted.
 * @author Harsh Sharma
 *
 */
public class DeleteNonVegItemsServlet extends HttpServlet {
	 ...
	 */
	static final Logger LOGGER = Logger.getLogger(DeleteNonVegItemsServlet.class);
	...
	/**
	 * doPost method that takes request from the user regarding which non vegetarian item has to be deleted.
	 * @param request A HttpServletRequest object
	 * @param response A HttpServletRequest object
	 * @throws ServletException Defines a general exception a servlet can throw when it encounters difficulty.
	 * @throws IOException Exceptions produced by failed or interrupted I/O operations
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			rowsUpdateCount = nonVegItemsService.deleteNonVegItems(itemNum);
			if (rowsUpdateCount > 0) {
				LOGGER.info("Non Veg Item is successfully deleted as per user's request.");
